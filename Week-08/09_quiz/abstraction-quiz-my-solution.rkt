...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname abstraction-quiz-my-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
;  PROBLEM 1:
;
...
(define (beside-all loi)
  (arrange beside (rectangle 0 0 "solid" "white") loi))


;; (Image Image -> Image) Image (listof Image) -> Image
;; arrange images according to fn
(check-expect (arrange above empty-image empty) empty-image)
(check-expect (arrange beside (rectangle 0 0 "solid" "white") empty) (rectangle 0 0 "solid" "white"))
(check-expect (arrange above empty-image (list (rectangle 20 40 "solid" "red") (star 30 "solid" "yellow")))
              (above (rectangle 20 40 "solid" "red") (star 30 "solid" "yellow")))
(check-expect (arrange beside empty-image (list (circle 10 "outline" "red") (circle 20 "outline" "blue") (circle 10 "outline" "yellow")))
              (beside (circle 10 "outline" "red") (circle 20 "outline" "blue") (circle 10 "outline" "yellow")))

(define (arrange fn b loi)
  (cond [(empty? loi) b]
        [else
         (fn (first loi)
             (arrange fn b (rest loi)))]))
...
(check-expect (fold-region make-region
                           cons
...
;; produce a list of all the names of all the regions in that region
...
(define (all-regions r)
  (local [(define (c1 name type los) (cons name los))]
